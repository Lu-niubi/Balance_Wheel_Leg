...
#define PITCH_OFFSET -6.32f//车往前走就调大（-8变成-7.5）车往后走就调小（-7.5变成-8）
// **************************** PIT中断函数 ****************************
...
// 先读字节喂给调参器
        uint8_t rx_byte = (uint8_t)Cy_SCB_ReadRxFifo(get_scb_module(UART_0));
        TuneCmd_Feed(rx_byte);
